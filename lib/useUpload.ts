...
import { encodeS3Component, normalizePrefix } from "./s3-prefix";
import { S3Credentials, UploadItem, UploadStatus } from "./types";
import { setUploadHeaders } from "./upload-header";
...
export function useUpload(
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        xhr.upload.addEventListener("progress", (e) => {
          ...
          if (e.lengthComputable) {
            updateItem(item.id, {
              progress: percent,
              status: percent === 100 ? "finishing" : "uploading",
            });
          }
        });
        ...
        setUploadHeaders(xhr, item, bucket, credentials);

        xhr.send(item.file); // file is the entire body, nothing else
      });
    },
  ...
  const clearDone = useCallback(
    () =>
      setItems((p) =>
        p.filter((i) => i.status === "pending" || i.status === "uploading"),
      ),
    [],
  );
